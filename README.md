...
> Redis **connectivity** is covered by this health endpoint when `REDIS_URL` is set. **Queue depth** and richer cache metrics may be extended later (optional counters).

### Invoice and dashboard read cache (FR43 / story 8.3)

When **`REDIS_URL` is set**, the API uses **shared Redis** for **cache-aside** on read-heavy endpoints. If Redis is **unreachable at process startup**, the cache layer is **disabled** and all reads go to PostgreSQL (no substitute in-memory cache for cross-instance consistency).

| Endpoint                        | Cached                                   | TTL (safety net) | Notes                                                                                           |
| ------------------------------- | ---------------------------------------- | ---------------- | ----------------------------------------------------------------------------------------------- |
| `GET /api/v1/dashboard/summary` | Yes                                      | 60               | Aggregates only; invalidated on invoice writes.                                                 |
| `GET /api/v1/invoices`          | Yes                                      | 25               | Response includes **draft** rows with line payloads → **short TTL** (architecture “Draft” row). |
| `GET /api/v1/invoices/:id`      | If status is **sent / paid / cancelled** | 60               | **Draft** responses are **never** stored in Redis.                                              |

**Key families (tenant-scoped):**

- `user:{userId}:dashboard:summary`
- `user:{userId}:invoices:list` and `user:{userId}:invoices:list*` (pattern delete for future filters)
- `user:{userId}:invoice:{invoiceId}`

**Invalidation (after successful DB commit, before HTTP response completes):** any invoice **create**, **metadata update** (draft), **lines replace** (draft), **status transition**, or **delete** triggers **`invalidateForUser(userId)`**, which deletes the user’s dashboard key, list key(s), and all cached invoice-detail keys for that user. **Read-your-writes** for the acting user is therefore satisfied when Redis is available.

**Not stored in Redis:** PDF binaries (architecture).

**Cache stampede:** not implemented for MVP (single-flight / mutex); document if you add high fan-in list endpoints.
