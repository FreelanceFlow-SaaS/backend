...
| ------------------------------- | ------------------------------------------------------------------------------------------------------- |
| **Structured JSON logs**        | Render Dashboard → **freelanceflow-api** → **Logs** tab — filter by `"request_id"` or `"level":"error"` |
...
2. Filter logs with: `{service="freelanceflow-api"}` — every line already contains `service`, `request_id`, `level`, `timestamp`
3. Recommended dashboards: **request rate** (`HTTP status` label), **error rate** (`level="error"`), **P95 latency** (from `responseTime` field)
