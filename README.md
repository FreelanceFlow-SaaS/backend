...
Every HTTP request carries a `X-Request-Id` header in the response (UUID generated by pino-http, or forwarded from an upstream gateway via the incoming `X-Request-Id` header). Use this ID to correlate log lines across services.

| Signal                 | Location                                                                                                            |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------- |
| **Structured logs**    | stdout (JSON in production, pretty in development) — ship to Loki via Promtail / Fluent Bit                         |
| **Grafana dashboards** | Configure Loki datasource pointing at your log aggregator; filter by `service="freelanceflow-api"` and `request_id` |
| **Render metrics**     | Available at **Dashboard → Your Service → Metrics** — CPU, memory, request rate, and error rate                     |
| **Error rate**         | Filter logs by `level="error"` in Loki; set alert threshold in Grafana                                              |
