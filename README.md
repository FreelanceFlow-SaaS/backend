...
| **Build version**               | `GET /api/v1/health` → `version` (`APP_VERSION` / package) and `gitSha` (commit SHA at Docker build)      |
| **Error rate**                  | Filter Render logs by `"severity":"error"`; set a Render notification alert for 5xx spikes                |
...
2. Filter logs with: `{service="freelanceflow-api"}` — every line already contains `service`, `requestId`, `severity`, and `timestamp` or `time`
3. Recommended dashboards: **request rate** (`HTTP status` label), **error rate** (`severity="error"`), **P95 latency** (from `responseTime` field)
